...
def iniciarServidor():
    ...
    while 1:
        ...
        threading.Thread(target= ouvirSocket, args=(connectionSocket,addr)).start()



def ouvirSocket(connectionSocket, addr):
	...
	print("Solicitado arquivo por: {}".format(addr))
    else:
	...
	dado = arquivo.read(4096)
	while(dado):
		connectionSocket.send(dado)
		dado = arquivo.read(4096)	

    	print("Arquivo enviado com sucesso para: {}".format(addr))
